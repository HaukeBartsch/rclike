<?php
 ...
 $pw_file = dirname(__FILE__)."/../passwords.json";
 $audit_file = dirname(__FILE__)."/../code/audit.log";
 $audit_on = 1;
  ...
  function audit( $what, $message ) {
    ...
    $callers = explode("\n", $callers);
    if (strpos( $callers[1], "/home/dataportal/www/code/php/AC.php" ) !== FALSE)
  ...
  function loadDB() {
     ...
     if (!file_exists($pw_file)) {
        echo ('error: permission file '. $pw_file. ' does not exist');
        return;
     }
  ...
  function addPermissionToRole( $permission, $role ) {
    ...
    if ($permission_id == -1) {
       echo("Error: no permission found !");
